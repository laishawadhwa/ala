...
def process_policy(image_path):
	car_brands=['HYUNDAI','TATA','MARUTISUZUKI','FORD','BMW','JEEP','MAHINDRA', 'HONDA','RENAULT','VOLKSWAGEN','AUDI','FIAT','NISSAN',\
				'TOYOTA','DATSUN','VOLVO','SKODA','MITSUBISHI']
    #reading file
	...
	if policy_location:
		    ...
		    min_x = policy_location.vertices[2].x + 45
	...
	policy_attr['policy_number']=policy_number.strip()
   
	RegNo=''
	#Registration Number
	Regno_location = find_compound_word_location(document,'Registration Number')
	if Regno_location:
		    #Extracting the Regno in next line in locality
	...
	if not Regno_location:
		...
		print("success")
		min_x = policy_location.vertices[2].x + 55
		max_x = policy_location.vertices[2].x + 20 * (policy_location.vertices[2].x-policy_location.vertices[0].x)
		min_y = policy_location.vertices[0].y + 3*(policy_location.vertices[2].y - policy_location.vertices[0].y)/2.0
		max_y = policy_location.vertices[2].y + (policy_location.vertices[2].y - policy_location.vertices[0].y)/2.0
		RegNo = text_within(document,min_x,min_y,max_x,max_y)
	...
	policy_attr['Registration Number']=RegNo.strip()
    #Make
	...
	print(make)
	if not make in car_brands:
		print("here")
		make_location = find_word_location(document,'Make/Model')
		if make_location:
			print("now here")
			min_x = make_location.vertices[2].x + 25
			max_x = make_location.vertices[2].x + 20 * (make_location.vertices[2].x-make_location.vertices[0].x)
			min_y = make_location.vertices[0].y + 3*(make_location.vertices[2].y - make_location.vertices[0].y)/2.0
			max_y = make_location.vertices[2].y + (make_location.vertices[2].y - make_location.vertices[0].y)/2.0
			make = text_within(document,min_x,min_y,max_x,max_y)
	print('Make : ', make)
	policy_attr['Make']=make.strip()
	#model
	model=''
	...
	print("body: ",body)   
	policy_attr['body']=body.strip()
	
	#chassis
	chassis=''
	chassis_location=find_compound_word_location(document,'Chassis Number')
	if chassis_location:
		min_x = chassis_location.vertices[0].x - 2*(chassis_location.vertices[2].x-chassis_location.vertices[0].x)
		max_x = chassis_location.vertices[2].x + 2.5*(chassis_location.vertices[2].x-chassis_location.vertices[0].x)/1.05
	...
	if not chassis_location:
		print("here")
		chassis_location=find_compound_word_location(document,'Chassis no/Engine no')
		min_x = policy_location.vertices[2].x + 35
		max_x = policy_location.vertices[2].x + 20 * (policy_location.vertices[2].x-policy_location.vertices[0].x)
		min_y = policy_location.vertices[0].y - (policy_location.vertices[2].y - policy_location.vertices[0].y)/2.0
		max_y = policy_location.vertices[2].y + (policy_location.vertices[2].y - policy_location.vertices[0].y)/2.0
		chassis = text_within(document,min_x,min_y,max_x,max_y)[:14].strip().rstrip('\r\n')
	    
	
	if chassis.find('/'):
		print("found // ", chassis)
	print("chassis number: ", chassis)
	policy_attr['Chassis Number']=chassis.strip()
	engine=''
	engine_location=find_compound_word_location(document,'Engine Number')

	if engine_location:
		min_x = engine_location.vertices[0].x -0.25*(engine_location.vertices[2].x-engine_location.vertices[0].x)
		max_x = engine_location.vertices[2].x + 2*(engine_location.vertices[2].x-engine_location.vertices[0].x)/1.05
	...
	policy_attr['Engine no']=engine.strip()
	...
	if date_location:
		min_x = date_location.vertices[2].x +80
		max_x = date_location.vertices[2].x + 35 * (date_location.vertices[2].x-date_location.vertices[0].x)
		min_y = date_location.vertices[0].y - (date_location.vertices[2].y - date_location.vertices[0].y)/0.5
		max_y = date_location.vertices[2].y + (date_location.vertices[2].y - date_location.vertices[0].y)/1.0
	...
	print("\n" , policy_attr)
	return policy_attr
process_policy('RC & Policy - Final-page-001.jpg') #('Insurance Policy Doc-1.jpg')
